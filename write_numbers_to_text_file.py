...
def get_entries():
    ...
    global min_number, max_number, label3_text, root
    ...
    #Both E1 and E2 have entries 
        ...
        if check_if_number(str(E2.get())) and check_if_number(str(E2.get())):
...
def add_to_text_file():
    ...
    global min_number, max_number, text_file_name

    done_writing = 0

    try:
        with open(text_file_name, 'a') as the_file:

            for i in range(min_number, int(max_number) + 1):
                the_file.write(str(i) + "\n")

    except:
        with open(text_file_name, 'w') as the_file:
...
#Root
root = Tk()
root.iconbitmap(default='transparent_icon.ico')
root.title('')
...
label3 = Label(root, textvariable=label3_text, borderwidth=10).grid(row=3,column=2)
...
#Button
submit = Button(root, text ="Enter", command = get_entries).grid(row=4,column=2, padx=10, pady=10)
